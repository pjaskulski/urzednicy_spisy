""" spis burgrabiow zamku krakowskiego  XIII-XV wiek """
import re
from pathlib import Path
import xml.dom.minidom
from burgrabiowie_krakow_literatura import literatura
...
class Position:
    def __init__(self, office:str='') -> None:
        self.office = office
        self.stated_in = 'Burgrabiowie Zamku Krakowskiego XIII-XV wieku, [w:] Urzędnicy dawnej Rzeczypospolitej XII-XVIII wieku, t. IV, Zeszyt 5, Kórnik 1999'
        self.id = ''
        self.reference_id = ''
        self.start_date = ''
        self.end_date = ''
...
def get_person(person_text: str, p_name:str='', p_herb:str='') -> Person:
    ...
    patterns = [
        ...
        r'\(\s?\d{4}-\d{1,2}\?\)', # (1656-67?)
        r'\(\s?\d{4}\?-\d{1,2}\)', # (1656?-67)
        r'\(ok\.\s+\d{4}\/\d{2}\)', # ok. 1445/50
        r'\([XVI]+\/[XVI]+\s+w\.\)', # (XVII/XVIII w.)
        r'\(\s?a\/l\.\s+\d{4}-\d{2}\s?\)', # (a/l. 1708-09)
        r'\(\s?błędnie:\s+[\d\s,]+\s?\)', # (błędnie: 1431, 1437)
        r'\(\d{4}\/\d{4}\)', # (1505/1506)
        r'\(\d{4}\s?-\s?ok\.\s+\d{4}\s?\)', # (1481-ok. 1490)
        r'\(\s?ok\.\s+\d{4}\s?\)', # (ok. 1480)
        r'\(\s?a\.\s?\d{4},\s+\d{4}\)', #  (a.1566, 1567)
        r'\(\d{4}-\d{4}\?\)', # (1698-1701?)
        ...
        r'\(\s?\d{4}\s+-\s+zm\.\s+\d{1,2}\s+[XVI]+\s+\d{4}\s?\)', # 1697 - zm. 1 II 1704
        r'\(\d{4}\s?,\s+\d{4}-\d{2}\s?\)', # (1441, 1448-50)
        r'\(\d{4}-\d{2}\s?,\s+\d{4}-\d{2}\s?\)', # (1375-78, 1381-87)
    ...
    # literatura w nawiasach na końcu?
    ...
    # podział na cześć z danymi postaci i część z listą urzędów
    pos = person_text.find('#')
    if pos == -1:
        print('ERROR: # not found')
        exit()
    else:
        positions_text = person_text[pos+1:].strip()
        person_text = person_text[:pos].strip()

    # location
    pattern_info = r'\{[a-zA-ZąśężźćńłóĄŚĘŻŹĆŃŁÓ\s\.,]+\}'
    matches = [x.group() for x in re.finditer(pattern_info, person_text)]
    for match in matches:
        tmp_location = match.strip().replace('{', '').replace('}', '')
        if ', z' in tmp_location:
            tmp_location = tmp_location[:-3]
            #tmp_location = 'z ' + tmp_location.strip()

        if urzednik.location:
            urzednik.location = urzednik.location +', ' + tmp_location
        else:
            urzednik.location = tmp_location

        # usunięcie z tekstu zalezionej informacji
        person_text = person_text.replace(match, '')

    # ród?
    pattern_info = r'\<[a-zA-ZąśężźćńłóĄŚĘŻŹĆŃŁÓ\s\.\(\),]+\>'
    matches = [x.group() for x in re.finditer(pattern_info, person_text)]
    for match in matches:
        tmp_rod = match.strip().replace('<', '').replace('>', '')
        urzednik.surname = tmp_rod
    ...
    pattern_info = r'\([a-zA-ZąśężźćńłóĄŚĘŻŹĆŃŁÓ\s\.\?,\d]+\)'
    matches = [x.group() for x in re.finditer(pattern_info, person_text)]
    for match in matches:
        tmp_info = match.strip().replace('(', '').replace(')', '')
        if 'z ' in tmp_info or 'von' in tmp_info:
            if urzednik.location:
                urzednik.location = urzednik.location +', ' + tmp_info
            else:
        ...
        # usunięcie z tekstu zalezionej informacji dodatkowej
        person_text = person_text.replace(match, '')
    ...
    if urzednik.coat_of_arms and urzednik.coat_of_arms in person_text:
        person_text = double_space(person_text.replace(urzednik.coat_of_arms, '').strip())
        if ' h.' in person_text:
            person_text = person_text.replace(' h.', '')
    ...
    if not p_name:
        ...
        if not urzednik.name and person_text.strip() != '':
    ...
    #     if match:
    #         if urzednik.location:
    #             urzednik.location = urzednik.location + ', ' + match.group().strip()
    #         else:
    ...
    #else:
    #    positions_text = person_text.strip()
    ...
    for position in tmp_positions:
        ...
        patterns_info = [r'o\.\s+a\.\s+\d{4}',
                         ...
                         r'zm\.\s+latem\s+\d{4}', # zm. latem 1746
                         r'zm\.\s+po\s+\d{2}-\d{2}\s+[XVI]+\s+\d{4}', # zm. po 26-30 IV 1436
                         r'zm.\s+\d{1,2}\s+[XVI]+\s+\d{4}\s+w\s+wieku\s+\d{2}\s+lat', # zm. 7 X 1710 w wieku 68 lat
        ...
        # jeżeli to informacja o pokrewieństwie
        is_relation_info = False
        relation_info = ['syn ', 'ojciec', 'brat', 'wnuk']
        for r_item in relation_info:
            if r_item in position:
                is_relation_info = True
                if urzednik.info:
                    urzednik.info += ', ' + position
                else:
                    urzednik.info = position
        ...
        if year:
            if "-" in year and ',' not in year:
                ...
                if len(urzad.end_date) == 2:
                    y_match = re.search(r'\d{4}', urzad.start_date)
                    if y_match:
                        tmp_start_date = y_match.group()
                    else:
                        tmp_start_date = urzad.start_date
                    urzad.end_date = tmp_start_date[:2] + urzad.end_date
        ...
        if len(t_office) > 1:
            ...
            if ((urzad.office.endswith('i') or urzad.office.endswith('.'))
                and urzad.office in skroty_geo):
...
if __name__ == '__main__':
    input_path = Path('.').parent / 'data/burgrabiowie_krakow.txt'
    output_path = Path('.').parent / 'output/burgrabiowie_krakow.xml'
