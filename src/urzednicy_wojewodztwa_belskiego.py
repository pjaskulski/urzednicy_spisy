""" spis urzędników centralnych polskich  XIV-XVIII wiek """
import re
from pathlib import Path
import xml.dom.minidom
from urzednicy_wojewodztwa_belskiego_literatura import literatura
...
class Position:
    def __init__(self, office:str='') -> None:
        self.office = office
        self.stated_in = 'Urzędnicy Województwa Bełskiego i Ziemi Chełmskiej XIV - XVIII wieku, [w:] Urzędnicy dawnej Rzeczypospolitej XII-XVIII wieku, t. II, Zeszyt 2, Kórnik 1992'
        self.id = ''
        self.reference_id = ''
        self.start_date = ''
        self.end_date = ''
...
def get_person(person_text: str, p_name:str='', p_herb:str='') -> Person:
    ...
    if p_name:
        ...
        if match:
            ...
            # wyjątki
            wyjatek = 'de Alten'
            if person_text.startswith(wyjatek):
                urzednik.surname += ' ' + wyjatek
                person_text = person_text.replace(wyjatek, '')
    ...
    for position in tmp_positions:
        ...
        relation_info = 'syn '
        if position.startswith(relation_info):
            if urzednik.info:
                urzednik.info += ', ' + position
            else:
                urzednik.info = position
...
if __name__ == '__main__':
    input_path = Path('.').parent / 'data/urzednicy_wojewodztwa_belskiego_xiv_xviii.txt'
    output_path = Path('.').parent / 'output/urzednicy_wojewodztwa_belskiego_xiv_xviii.xml'
